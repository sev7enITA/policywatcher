...
| `/api/v1/publication-readiness` | `GET` | One authoritative database-derived funnel: configured, retrieved, baseline verified, public and analysed, plus the latest successful capture timestamp. Schema: `/schemas/publication-readiness/v1`. |
...
| `/api/v1/integrations/palo/signals?limit=25&lang=en` | `GET` | No-store, complete active snapshot pagination for the optional PALO pull transport. Accepts `cursor`, `limit` and `lang`; every embedded signal retains the PALO-owned non-authoritative review boundary. |
| `/api/v1/release-evidence` | `GET` | Versioned fourteen-day release evidence ledger with implementation impact, metrics, residual boundaries, ETag and a deterministic SHA-256 digest. |
...
All endpoints support read-only browser access with no credentials and send `Access-Control-Allow-Origin: *` without accepting cookies or credentials. The manifest, Observatory, Webhook Verification Kit and Receiver Conformance Suite use 60 request-per-minute reference-route buckets. Evidence Collections, Change Events, the PALO Signal Pull Transport and the Agent Evidence Gateway each use a separate 30 request-per-minute bucket. Agent requests do not log the client IP. Collections and Release Evidence use a 300-second browser cache and a 3,600-second shared-cache window; Change Events and agent briefs use short public caching; the PALO transport is no-store so a withdrawn public-evidence record is visible on the next complete traversal; the versioned receiver resources use a 3,600-second browser cache and an 86,400-second shared-cache window.

Publication Readiness is calculated on request by one server-side data service and is returned with `Cache-Control: no-store`. Its denominator is the configured policy inventory. “Retrieved” requires a successful non-seeded check with persisted text or hash evidence; “baseline verified” requires a snapshot marked as public evidence; “public” applies the production publication gate; “analysed” additionally requires a change backed by public evidence. “Latest capture” is the newest retrieval satisfying the same retrieved-evidence rule. A missing or failed query remains explicitly unavailable and is never converted to zero.

The payload identifies `https://policywatcher.online/schemas/publication-readiness/v1`, contract `1.0.0` and source `database`. The five stage IDs are ordered and stable: `configured`, `retrieved`, `baseline-verified`, `public`, `analysed`. Counts may be `null` only when their availability is explicit; clients must not coerce unavailable measurements to zero.

The Release Evidence route is a static, public implementation ledger. Its ETag is derived from the ledger digest and supports `If-None-Match`. The listed metrics are release-scope inventory or validation observations and must not be interpreted as adoption, business impact, service availability, legal compliance or third-party attestation.
...
- Publication Readiness exposes aggregate counts and one timestamp only. It is not proof of exhaustive source coverage, legal review, current provider availability or analysis quality.
