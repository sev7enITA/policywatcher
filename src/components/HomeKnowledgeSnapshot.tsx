...
export default function HomeKnowledgeSnapshot({ data }: HomeKnowledgeSnapshotProps) {
  return (
    <section className={styles.section} aria-label="Public policy records">
      <details className={styles.disclosure} open>
        <summary>
          <span>
            <span className={styles.kicker}><BookOpenCheck size={14} aria-hidden="true" /> Public policy records</span>
        ...
        <div className={styles.body}>
          <div className={styles.header}>
            <div>
              <h2>Explore company policies and recorded updates.</h2>
              <p>Browse company records, policy sources and recorded versions. Each update links to its evidence so you can check the source and the limits of the comparison.</p>
            </div>
          ...
          {!data ? (
            ...
            <>
              ...
              <div className={styles.indexes}>
                ...
                <div><strong>Policy records</strong><nav aria-label="Sample public policy records">{data.policies.slice(0, 5).map((policy) => <Link key={policy.id} href={`/knowledge/companies/${policy.company.slug}/policies/${policy.id}`}>{policy.company.name} / {policy.name} ({policy.jurisdiction})</Link>)}</nav></div>
              </div>
