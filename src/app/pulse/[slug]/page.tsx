...
export default async function PulseStoryPage({ params, searchParams }: Props) {
  ...
  return <div className={styles.page} lang={lang}><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd).replace(/</g, '\\u003c') }} /><PulseStoryViewTracker story={story} lang={lang} /><PublicHeader current="pulse" lang={lang} lockLang /><main className={styles.storyMain}>
    ...
    <article><header className={styles.storyHero}><p className={styles.kicker}>VERIFIED STORY LEAD · PACK v{story.version}</p><h1>{story.headline[lang]}</h1><p id="story-deck">{story.deck[lang]}</p><dl><div><dt>Beat</dt><dd>{pulseBeatLabels[story.beat][lang]}</dd></div><div><dt>Status</dt><dd><CheckCircle2 size={14} />Verified</dd></div><div><dt>As of</dt><dd>{story.asOf}</dd></div><div><dt>Pack</dt><dd>v{story.version}</dd></div></dl></header>
      <div className={styles.storyLayout}><div className={styles.storyContent}>
        <section><h2>{lang === 'en' ? 'Editorial context' : 'Contesto editoriale'}</h2><p className={styles.largeText}>{story.whyItMatters[lang]}</p></section>
        ...
        <section className={styles.caveat}><ShieldAlert size={22} /><div><h2>{lang === 'en' ? 'Scope and limitations' : 'Ambito e limitazioni'}</h2><p>{story.boundary[lang]}</p></div></section>
      ...
      </div><aside className={styles.reusePanel}><p className={styles.kicker}>{lang === 'en' ? 'STORY ASSETS' : 'MATERIALI EDITORIALI'}</p><h2>{lang === 'en' ? 'Files, cards and citation' : 'File, card e citazione'}</h2><p>{lang === 'en' ? 'The files and actions refer to this fixed story version.' : 'I file e le azioni fanno riferimento a questa versione fissa della storia.'}</p><PulseActions story={story} lang={lang} /><nav><Link href="/press-kit/data">Data Room</Link><Link href="/press-kit#claim-registry">Claim Registry</Link></nav></aside></div>
    </article></main><Footer lang={lang} variant="compact" lockLang /></div>;
}
