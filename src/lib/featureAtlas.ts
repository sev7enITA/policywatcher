...
export const FEATURE_ATLAS_RELEASES: FeatureAtlasRelease[] = [
  ...
  { id: '3.9.0-beta.30', shortLabel: '3.9 B30', label: '3.9.0 Beta 30' },
  { id: '3.9.0-beta.31', shortLabel: '3.9 B31', label: '3.9.0 Beta 31' },
  { id: '3.9.0-beta.32', shortLabel: '3.9 B32', label: '3.9.0 Beta 32' },
  { id: '3.9.0-beta.33', shortLabel: '3.9 B33', label: '3.9.0 Beta 33' },
  ...
  { id: '3.9.0-beta.39', shortLabel: '3.9 B39', label: POLICYWATCHER_VERSION_DISPLAY },
].map((release) => ({
  ...release,
...
const routeByFeature: Record<string, FeatureAtlasRoute> = {
  ...
  'residency-assurance': { href: '/trust/residency', label: 'Residency evidence', access: 'public' },
  'production-validation': { href: '/admin/production-verification', label: 'Production verification', access: 'protected' },
  'renderer-hardening': { href: '/admin/vps-services', label: 'VPS services', access: 'protected' },
  'managed-vps-releases': { href: '/admin/vps-services', label: 'VPS managed releases', access: 'protected' },
  'source-remediation-workbench-ux': { href: '/admin/source-reliability', label: 'Source Remediation Workbench', access: 'protected' },
  'community-signal-composer': { href: '/roadmap', label: 'Community roadmap', access: 'public' },
  'admin-mutation-hardening': { href: '/security', label: 'Security policy', access: 'public' },
...
const dependencyByFeature: Record<string, FeatureAtlasDependency[]> = {
  ...
  'managed-vps-releases': [{ featureId: 'renderer-hardening', relationship: 'depends-on' }, { featureId: 'admin-operations', relationship: 'distributed-through' }],
  'source-remediation-workbench-ux': [{ featureId: 'source-reliability-control-plane', relationship: 'depends-on' }, { featureId: 'admin-operations', relationship: 'distributed-through' }],
  'community-signal-composer': [{ featureId: 'impact-clarity', relationship: 'depends-on' }, { featureId: 'interactive-public-navigation', relationship: 'distributed-through' }],
  'admin-mutation-hardening': [{ featureId: 'request-hardening', relationship: 'depends-on' }, { featureId: 'security-privacy-boundaries', relationship: 'governed-by' }],
...
const platformSurfaceFeatures: FeatureAtlasFeature[] = [
  ...
  surfaceFeature({
    ...
    benefit: 'Operators receive measured integrity, table, migration-ledger and file-access signals instead of an opaque application failure.', kpi: 'Inventory KPI · protected database readiness check available', kri: 'Residual KRI · post-deploy execution and restore success still require operator verification', evidence: 'Protected read-only readiness endpoint and panel, bounded diagnostic codes, schema-parity tests, deployment diagnostic and initializer scripts.', limitation: 'The readiness panel does not migrate, reset, repair or restore production data.', primaryUser: 'Deployment operator', route: { href: '/admin/database', label: 'Database operations', access: 'protected' }, dependencies: [{ featureId: 'hostinger-recovery', relationship: 'depends-on' }],
  }),
  surfaceFeature({
