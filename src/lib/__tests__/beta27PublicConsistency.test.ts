...
describe('Beta 42 public consistency', () => {
  it('keeps one current release across release, impact, atlas and newsroom records', () => {
    expect(POLICYWATCHER_VERSION).toBe('3.9.0-beta.42');
    expect(POLICYWATCHER_RELEASE_NAME).toBe('Evidence Release Control Plane');
    expect(POLICYWATCHER_RELEASE_DATE).toBe('2026-08-15');
  ...
  it('publishes prior waves and the current evidence control plane', () => {
    ...
    expect(RELEASE_IMPACT_ITEMS.some((item) => item.id === 'ai-evalops-control-plane' && item.status === 'current' && item.startRelease === '3.9.0-beta.42')).toBe(true);
