...
describe('Beta 40 public consistency', () => {
  it('keeps one current release across release, impact, atlas and newsroom records', () => {
    expect(POLICYWATCHER_VERSION).toBe('3.9.0-beta.40');
    expect(POLICYWATCHER_RELEASE_NAME).toBe('PolicyWatcher Civico');
    expect(POLICYWATCHER_RELEASE_DATE).toBe('2026-08-06');
  ...
  it('publishes prior waves and the current consumer-association workspace', () => {
    ...
    expect(RELEASE_IMPACT_ITEMS.some((item) => item.id === 'consumer-association-civic-workspace' && item.status === 'current' && item.startRelease === '3.9.0-beta.40')).toBe(true);
