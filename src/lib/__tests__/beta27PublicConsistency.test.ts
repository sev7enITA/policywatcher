...
describe('Beta 37 public consistency', () => {
  it('keeps one current release across release, impact, atlas and newsroom records', () => {
    expect(POLICYWATCHER_VERSION).toBe('3.9.0-beta.37');
    expect(POLICYWATCHER_RELEASE_NAME).toBe('Resource Navigation & Retrieval Diagnostics');
  ...
  it('publishes the prior waves and the current navigation and retrieval diagnostics', () => {
    ...
    expect(RELEASE_IMPACT_ITEMS.some((item) => item.id === 'categorized-resource-navigation' && item.status === 'current' && item.startRelease === '3.9.0-beta.37')).toBe(true);
    expect(RELEASE_IMPACT_ITEMS.some((item) => item.id === 'retrieval-deduplication-diagnostics' && item.status === 'current' && item.startRelease === '3.9.0-beta.37')).toBe(true);
    ...
    expect(read('scripts/package-release.sh')).toContain('docs/audit-v3.9.0-beta.37.md');
  });
});
