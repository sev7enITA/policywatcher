...
describe('release metadata', () => {
  it('matches the package version and composes one build label', () => {
    ...
    expect(POLICYWATCHER_RELEASE_NAME).toBe('Citable Coverage Registry');
    expect(POLICYWATCHER_BUILD_LABEL).toBe('v3.9.0-beta.15 Citable Coverage Registry');
    ...
    expect(POLICYWATCHER_RELEASE_BADGE).toBe('v3.9.0-beta.15 · BETA');
