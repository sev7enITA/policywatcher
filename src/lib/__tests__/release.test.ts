...
describe('release metadata', () => {
  it('matches the package version and composes one build label', () => {
    ...
    expect(POLICYWATCHER_RELEASE_NAME).toBe('Editorial Pulse and Distribution');
    expect(POLICYWATCHER_BUILD_LABEL).toBe('v3.9.0-beta.13 Editorial Pulse and Distribution');
    ...
    expect(POLICYWATCHER_RELEASE_BADGE).toBe('v3.9.0-beta.13 · BETA');
