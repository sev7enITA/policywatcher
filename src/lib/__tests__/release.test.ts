...
describe('release metadata', () => {
  it('matches the package version and composes one build label', () => {
    ...
    expect(POLICYWATCHER_RELEASE_NAME).toBe('Evidence Workflow Refinements');
    expect(POLICYWATCHER_BUILD_LABEL).toBe('v3.9.0-beta.18 Evidence Workflow Refinements');
    ...
    expect(POLICYWATCHER_RELEASE_BADGE).toBe('v3.9.0-beta.18 · BETA');
